...
def home():
    # Montar pantalla
    home_label.config(fg="white", bg="black", font=("Arial", 30), padx=190, pady=20)
...
def add():
    # Encabezado
    add_label.config(fg="white", bg="black", font=("Arial", 30), padx=80, pady=20)
    ...
    add_description_entry.config(
        width=30, height=5, font=("Consolas", 12), padx=15, pady=15
    )
...
def info():
    info_label.config(fg="white", bg="black", font=("Arial", 30), padx=120, pady=20)
...
def add_product():
    products.append(
        [name_data.get(), price_data.get(), add_description_entry.get("1.0", "end-1c")]
    )
    name_data.set("")
    price_data.set("")
...
# Definic campos de pantalla (ADD)
...
data_label = Label(ventana, text="Creado por Víctor Robles - 2021")
